...
function PetCard({ idPet }) {
    ...
    useEffect(() => {
        const getMessage = async () => {
            try {
                const response = await axios.get(`http://localhost/Dog_adoption_website-main/api/index.php`);
            ...
            {donnees.map((pet) => (
                <div key={pet.id} className="card" style={{width:"350px" , marginLeft:"60px" ,}} >
                    ...
                    <div className="card_info" style={{backgroundColor:"#EEEEEE",padding:"25px",}}>
                        <span className="Dog_name" style={{color:"#CF0A0A",letterSpacing:"5px",}}><b style={{fontSize:"20px",}}><u>{pet.genre}</u></b></span>
                        <br />
                        <span className="Dog_info"><b style={{color:"#CF0A0A"}}> Sexe: </b> {pet.sexe}</span>
                        <br />
                        <span className="Dog_info"><b style={{color:"#CF0A0A"}}> Description: </b> {pet.description}</span>
                        <br />
                        <span className="Dog_info"><b style={{color:"#CF0A0A"}}> Age: </b>  {pet.age} </span>
                        <br />
